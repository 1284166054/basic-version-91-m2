...
exports.requestcastlewarnow = exports.requestcastlewar = exports.warrule = exports.guildwar2 = exports.guildwarexp = exports.buildguildexp = exports.InPutString02 = exports.InPutString01 = exports.guildwar = exports.Main = exports.GuildWarPrice = exports.CreateGuildGold = exports.WomaHorn = void 0;
//行会
exports.WomaHorn = '沃玛号角';
exports.CreateGuildGold = 1000000; //创建行会费用
exports.GuildWarPrice = 100000; //行会战申请费用   
...
exports.Main = Main;
function guildwar(Npc, Player, Args) {
    const S = `
填写与你交战的敌对行会的名字，申请行会战争必须支付${exports.GuildWarPrice}{金币\\\\
<立即申请行会战争/@@InPutString02(请输入对方行会名称:)>\\
<返回/@main>\
`;
    Npc.Say(Player, S);
}
exports.guildwar = guildwar;
function InPutString01(Npc, Player, Args) {
    let AGuildName = '';
    let AGuild;
    ...
    if (Player.Gold < exports.CreateGuildGold) {
        Npc.MessageBox(Player, `创建行会需要金币{S=${String(exports.CreateGuildGold)};C=249}`);
        return;
    }
    if (Player.GetItemCount(exports.WomaHorn) == 0) {
        Npc.MessageBox(Player, `创建行会需要{S=${exports.WomaHorn};C=249}`);
        return;
    }
    ...
    if (AGuild != null) {
        Npc.Take(Player, '金币', exports.CreateGuildGold);
        Npc.Take(Player, exports.WomaHorn, 1);
...
exports.InPutString01 = InPutString01;
function InPutString02(Npc, Player, Args) {
    let AGuildName = '';
    if (Player.Guild == null) {
        return;
    }
    ...
    if (Player.Gold < exports.GuildWarPrice) {
        Npc.MessageBox(Player, `申请行会战需要金币{S=${String(exports.GuildWarPrice)};C=249}`);
        return;
    }
    ...
    if (GameLib.AddGuildWar(Player.Guild.Name, AGuildName)) {
        Npc.Take(Player, '金币', exports.GuildWarPrice);
        Npc.MessageBox(Player, '行会战申请成功！');
        Npc.CloseDialog;
    }
}
exports.InPutString02 = InPutString02;
function buildguildexp(Npc, Player, Args) {
    const S = `
...
exports.buildguildexp = buildguildexp;
function guildwarexp(Npc, Player, Args) {
    const S = `
...
exports.guildwarexp = guildwarexp;
function guildwar2(Npc, Player, Args) {
    const S = `
...
exports.guildwar2 = guildwar2;
function warrule(Npc, Player, Args) {
    const S = `
...
exports.warrule = warrule;
function requestcastlewar(Npc, Player, Args) {
    const S = `
...
exports.requestcastlewar = requestcastlewar;
function requestcastlewarnow(Npc, Player, Args) {
...
exports.requestcastlewarnow = requestcastlewarnow;
//# sourceMappingURL=%E6%AF%94%E5%A5%87%E5%9B%BD%E7%8E%8B.js.map