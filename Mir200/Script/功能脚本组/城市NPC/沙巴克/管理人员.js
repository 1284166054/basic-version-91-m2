...
exports.commandorder = exports.archersell = exports.archerbuy = exports.hirearchers = exports.guardsell = exports.guardbuy = exports.hireguards = exports.guardcmd = exports.repairdoor = exports.repaircastle = exports.openmaindoor = exports.closemaindoor = exports.treatdoor = exports.InputInteger2 = exports.InputInteger1 = exports.Main = void 0;
...
function treatdoor(Npc, Player, Args) {
    let S = `
...
exports.treatdoor = treatdoor;
function closemaindoor(Npc, Player, Args) {
    Npc.Castle.CloseDoor();
    treatdoor(Npc, Player, Args);
    Npc.MessageBox(Player, `城门已关闭.\\\\`);
}
exports.closemaindoor = closemaindoor;
function openmaindoor(Npc, Player, Args) {
    Npc.Castle.OpenDoor();
    treatdoor(Npc, Player, Args);
    Npc.MessageBox(Player, `城门已打开.\\\\`);
}
exports.openmaindoor = openmaindoor;
function repaircastle(Npc, Player, Args) {
...
exports.repaircastle = repaircastle;
function repairdoor(Npc, Player, Args) {
...
exports.repairdoor = repairdoor;
function guardcmd(Npc, Player, Args) {
    let S = `
你想你要聘用守卫吗?\\
<聘用流动护卫/@hireguards>\\
<聘用固定弓箭手/@hirearchers>\\
...
exports.guardcmd = guardcmd;
function hireguards(Npc, Player, Args) {
...
exports.hireguards = hireguards;
function guardbuy(Npc, Player, Args) {
...
exports.guardbuy = guardbuy;
function guardsell(Npc, Player, Args) {
    let index = Args.Int[0];
    if (Npc.Castle.GetGuard(index) == undefined) {
...
exports.guardsell = guardsell;
function hirearchers(Npc, Player, Args) {
    let ArcherList = new Array(16);
...
exports.hirearchers = hirearchers;
function archerbuy(Npc, Player, Args) {
    let index = Args.Int[0];
...
exports.archerbuy = archerbuy;
function archersell(Npc, Player, Args) {
    let index = Args.Int[0];
    if (Npc.Castle.GetArcher(index) == undefined) {
...
exports.archersell = archersell;
function fn(num, length = 2) {
    return (Array(length).join('0') + num).slice(-length);
}
function commandorder(Npc, Player, Args) {
    Npc.MessageBox(Player, "暂不支持的功能");
}
exports.commandorder = commandorder;
//# sourceMappingURL=%E7%AE%A1%E7%90%86%E4%BA%BA%E5%91%98.js.map